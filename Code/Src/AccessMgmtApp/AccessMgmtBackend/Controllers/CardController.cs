...
namespace AccessMgmtBackend.Controllers
{
    ...
    public class CardController : ControllerBase
    {
        ...
        public IEnumerable<AppUser> AssetToUsers(string companyId, string assetId)
        {
            ...
            {
                var listOfMappings = _companyContext.AssetToUsers.Where(x => x.company_identifier == companyId && x.asset_identifier == assetId && x.is_approved == true)?
        ...
        public IEnumerable<Role> AssetToRoles(string companyId, string assetId)
        {
            ...
            if (!string.IsNullOrEmpty(companyId) && !string.IsNullOrEmpty(assetId))
            {
                var listOfMappings = _companyContext.AssetToRoles.Where(x => x.company_identifier == companyId && x.asset_identifier == assetId && x.is_approved == true)?
        ...
        public IEnumerable<Employee> AssetToEmployees(string companyId, string assetId)
        {
            ...
            if (!string.IsNullOrEmpty(companyId) && !string.IsNullOrEmpty(assetId))
            {
                var listOfMappings = _companyContext.AssetToEmployees.Where(x => x.company_identifier == companyId && x.asset_identifier == assetId && x.is_approved == true)?
                ...
                foreach (var employee in listEmployees)
                {
                    ...
                    employee.emp_role = String.Join(",",
                        ...
                        (x => x.company_identifier == companyId && x.employee_identifier == employee.employee_identifier.ToString() && x.is_approved == true).
